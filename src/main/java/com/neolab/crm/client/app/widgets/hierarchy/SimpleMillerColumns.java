...
public class SimpleMillerColumns<E> extends MillerColumnsWidget implements HasValueChangeHandlers<E> {
    ...
    public static class SelectableLabel<E> extends HTML implements ClickHandler {
        ...
        private SelectableLabel(E element, int column, String displayName, String icon, SimpleMillerColumns smc) {
            if (icon == null || icon.isEmpty()) {
                ...
                html
                        .append("<div style='background:url(")
                        .append(icon)
                        .append(") no-repeat left center;padding-left:17px;'>");
    ...
    private static class HeaderWidget extends Composite {
        ...
        private NavButton up = new NavButton(new Image("images/arrow_up.gif"),
                new Image("images/arrow_up.gif"), new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                        previous();
                    }
                });
        private NavButton down = new NavButton(new Image("images/arrow_down.gif"),
                new Image("images/arrow_down.gif"), new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                        next();
                    }
    ...
    public void show() {
//        Log.debug("SimpleMillerColumns -> show");
        resetVariables();
...
//                Log.debug("SimpleMillerColumns -> onFailure " + caught);
                loadingWidget.hide();
            }
...
//                    Log.debug("SimpleMillerColumns -> onSuccess " + result.total);
                    ...
                    final E toSelect = !selectFirstItemOnInit ? null
                            : firstColumnElements.size() == 0 ? null
                            : firstColumnElements.get(0);
...
//                    Log.debug("SimpleMillerColumns -> There no results so reset widget");
                    reset();
                }
                loadingWidget.hide();
            }
...
//                    GWT.log("failure", null);
    ...
    public void changeLevel(final int level, E root1, E root2) {
        ...
        if (levelStateHolder != null)
            changeContent(level, levelStateHolder);
        else {
            assert root2 != null;
                    ...
                    if (result != null) {
                        ...
                        if (col2Parent != null) {
                            model.search(col2Parent, 0, itemsPerPage, new AsyncCallback<SearchResult<E>>() {
                                ...
                                public void onFailure(Throwable caught) {
//                                    Log.debug("failure");
                                    LevelStateHolder<E> holder = new LevelStateHolder<E>(selected, col1Parent, col2Parent,
                                            firstColumnElements, secondColumnElements, col1Page, col2Page, col1Total, col2Total);
...
//                        Log.debug("Zero children for " + model.getDisplayString(col1Parent));
                    }
                    loadingWidget.hide();
                }
            });
